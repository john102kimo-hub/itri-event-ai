...
`vo.py` 跑完會印出每句長度，超過 4.9 秒就把句子砍短（改 `LINES`），不要只調語速——
語速再快會變得不像人講話。
...
ffmpeg -y -i bgm.wav -i vo0.mp3 -i vo1.mp3 -i vo2.mp3 -i vo3.mp3 -i vo4.mp3 -i vo5.mp3 \
  -filter_complex "\
...
[vo]volume=1.6,alimiter=limit=0.95,atrim=0:30[voL];\
