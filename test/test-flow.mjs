...
check('職員功能表要寫出【教米亞】（批次 46 做好了，除了這裡沒有別的地方寫著怎麼叫它）',
  out.some(o => o.kind === 'text' && /教米亞/.test(o.text)), JSON.stringify(out));

// 批次 55：拿掉【記者問得到的，您一樣問得到】那一段。原話「職員模式應該重點是調整、
// 設定？要問趨勢什麼的去跟記者一樣地方就好，不用進入職員模式吧」——這份清單要回答的是
// 「在這個模式裡我能做什麼」，列一串「其實你不用進來也做得到」的東西只是把它撐長。
check('職員功能表不再列記者端那些功能（趨勢／技術／邀訪窗口）',
...
// 這段文字。下一個人讀到「職員功能表不該列記者端的東西」，很容易順手把路由也清掉——
// 回報過的「問新聞稿拿到一面功能清單的牆」就會整個回來。
//
// 守這件事的是**上面**兩組測試，不是這裡再補一條：
//   ① 情境 6.5（新聞／趨勢／邀訪窗口）——驗有沒有真的去抓 IEK 與官網新聞
//   ② 下面「職員講了聽不懂的話 → 不再丟一面功能清單的牆」——那一條是把 'other' 改回
//      丟功能表時唯一會紅的測試，也就是「改回只做管理」這個回歸的守門員
// 兩種壞法都實際弄壞驗過會紅。
//
// 這裡刻意**不**再補一條「職員問趨勢照樣答得出來」：寫過一版，用的是「產業趨勢分析」，
// 結果兩種壞法它都照樣綠——因為那句話有短路與兜底兩層保護，弄壞一層另一層就接住了。
// 一條永遠不會紅的測試比沒有測試更糟，它讓整份測試看起來有守住其實沒有。
