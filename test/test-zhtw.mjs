...
console.log('── 歧義字出現在整段文字的開頭或結尾 ──');
{
  // ⚠️ 這一組是真的漏過字。S2T_CONTEXT 那四個歧義字（后／发／准／里）本來寫成
  // `keepPrev.includes(prev)`，而字串開頭時 prev 是空字串——JS 的
  // String.includes('') **恆為 true**，所以只要這幾個字剛好落在整段文字的頭或尾，
  // 就一律被當成例外、完全不轉。
  //
  // 其中「发」最嚴重：它在繁體中文裡根本不是一個字（發／髮才是），漏掉就是**簡體字
  // 真的送到記者眼前**——正是這支檔案存在的唯一理由。
  for (const [s, want] of [
    ['发', '發'],
    ['发人深省', '發人深省'],
    ['请勿转发', '請勿轉發'],          // 以「发」收尾
    ...
    check(`開頭／結尾的歧義字有轉：「${s}」→「${want}」`, toTraditionalTW(s) === want,
      `實際：「${toTraditionalTW(s)}」`);
  }

  // 修完之後，原本就該保留的例外一個都不能壞——這才是這條規則存在的意義
  for (const keep of ['批准', '不准', '准許', '皇后', '太后', '后妃', '公里', '里程',
    '鄉里', '頭髮', '髮型', '獲准', '核准']) {
    check(`例外仍然保留：「${keep}」不被改`, toTraditionalTW(keep) === keep,
      `實際：「${toTraditionalTW(keep)}」`);
  }
}
