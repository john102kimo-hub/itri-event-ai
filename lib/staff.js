...
// ⚠️ 批次 80：追問狀態一律讀最新的，不吃 60 秒快取。上面那段註解講的就是這件事——
// 「下一則訊息不保證落在同一個 instance」——但舊版讀的是 getAllStaffRows() 的快取：
// 寫入時只清得到自己這個 instance 的快取，下一則落到別的 instance，讀到的還是 60 秒前
// 的空白，同仁按「✅ 確認修改」只會拿到「沒有等著確認的修改」。多一次 Sheets 讀取，
// 換「追問一定接得住」；職員的訊息量很小，划得來。
async function freshStaffRow(userId) {
  let rows = [];
  try { rows = await readRange(LINE_STAFF_RANGE); } catch { return null; }
  return rows.find(r => r[0] === userId && r[3] !== REVOKED) || null;
}

export async function getStaffPending(userId) {
  const row = await freshStaffRow(userId);
  const raw = row?.[4] || '';
  if (!raw) return null;
...
// 最近辦完的接在後面，封存的不列。
//
// 批次 80（回報：「要修改活動，不要跳出已經過期的活動」）：舊版把「日期已過的草稿」也當成
// 接下來要辦的（sortUpcoming() 對草稿一律放行），又把辦完的場次接在後面，於是「更新活動」
// 第一顆按鈕就是上個月沒發的草稿。現在「過期」只看日期：有真的活動日期、而且早於今天，
// 不管什麼狀態都算過期。
//   includePast：要不要把過期的接在後面。改資料、發布、要訓練連結都不要（false）；
//                查成效、補照片是活動後才做的事，要（true）
//   pastDays   ：過期的只收最近幾天（補照片只收兩週內）
export function isExpiredCard(c, today = startOfTaipeiToday()) {
  return !!(c && c.date && !c.date_tbd && c.date < today);
}
function startOfTaipeiToday() {
  const [y, m, d] = todayTaipei().split('-').map(Number);
  return new Date(y, m - 1, d);
}
export function staffPickList(cards, limit = 8, { includePast = true, pastDays = Infinity } = {}) {
  const today = startOfTaipeiToday();
  const live = (cards || []).filter(c => c && c.status !== 'archived');
  const realDate = c => (c.date && !c.date_tbd ? c.date.getTime() : Infinity);
  const upcoming = live
    .filter(c => !isExpiredCard(c, today) && c.status !== 'ended')
    .sort((a, b) => realDate(a) - realDate(b));
  const oldest = today.getTime() - pastDays * 86400000;
  const past = includePast
    ? live.filter(c => isExpiredCard(c, today) && c.date.getTime() >= oldest)
...
export async function getGeoStatusSummary() {
  ...
  try {
    const res = await fetch(`${SITE}/api/geo?action=status&password=${encodeURIComponent(admin)}`, { signal: AbortSignal.timeout(15_000) });
...
export async function getGeoTrendSeries(days = 14) {
  ...
  if (!admin) return null;
  try {
    const res = await fetch(`${SITE}/api/geo?action=series&password=${encodeURIComponent(admin)}&days=${days}`, { signal: AbortSignal.timeout(15_000) });
    if (!res.ok) return null;
    return await res.json();
  } catch (e) {
...
{"intent":"calendar|progress|update_event|publish|qa|geo_status|event_analytics|create_event|training_link|setup_richmenu|other","event_ids":["qa/event_analytics/training_link/update_event/publish 用，從清單挑 id，最多3個"],"new_event_name":"create_event 才有，新活動名稱","new_event_date":"create_event 才有，YYYY-MM-DD，訊息沒提到就留空字串","update_field":"update_event 才有：name|date|time|venue|press_contact 五選一","update_value":"update_event 才有：改成的新值（date 用 YYYY-MM-DD，其餘照原話）","confidence":"high|low"}`;
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      // 批次 82：這支原本沒有逾時。模型那端不回應時，職員這則訊息會一路等到 Vercel 在 60 秒
      // 把整支砍掉——已讀不回，連 api/line.js 的 apologise() 都沒機會跑。跟記者那條路的
      // lib/router.js 一樣給 15 秒，逾時走下面的 catch（照舊退回「其他」意圖）。
