...
// ── GEO 趨勢圖：真的傳一張圖，而不是只給一個要點密碼才進得去的連結 ──────────
// 用 QuickChart.io 的公開 GET API 把資料轉成 PNG：直接組一個 URL，LINE 的伺服器
// 會自己去抓圖，我們不用另外跑一支圖片生成服務、也不用碰 headless browser 或
// canvas 這類在 Vercel Serverless Function 裡容易出包的原生依賴。
//
// ⚠️ 這條 URL 刻意不帶 ADMIN_PASSWORD：LINE image 訊息的 originalContentUrl 必須是
// 「任何人／任何伺服器」都能直接 GET 到的網址（LINE 自己的伺服器會去抓，不會帶我們
// 的密碼），把密碼放進網址就等於把後台密碼寫死在一個會被 LINE CDN 快取、可能被轉傳
// 的連結裡。改用 series 資料本身（AI 能見度分數的走勢）當作可公開的內容——這不是
// 活動的新聞稿或個資，跟 /event 頁面本來就對外開放的道理一樣；真正機密的判官設定、
// CRON_SECRET 仍然留在需要密碼的 action=status／series 端點後面。
const QUICKCHART_BASE = 'https://quickchart.io/chart';
const CHART_DAYS = 14;

export async function getGeoTrendSeries(days = CHART_DAYS) {
  const admin = process.env.ADMIN_PASSWORD;
  if (!admin) return null;
  try {
...
// 純函式、不碰網路——series 資料組好之後怎麼變成一張圖，邏輯全部收在這裡才測得到。
// 回傳 null 代表「還沒有足夠資料畫圖」（例如剛啟用、一次掃描都還沒跑過），呼叫端
// 看到 null 就不要送圖片訊息，不然 LINE 會顯示一張空白或壞掉的圖，比不傳還糟。
export function buildGeoTrendChartUrl(seriesData) {
  const dates = seriesData?.series?.dates;
  const ma = seriesData?.series?.overall?.ma;
  if (!Array.isArray(dates) || !Array.isArray(ma) || !dates.length) return null;
  if (!ma.some(v => v !== null && v !== undefined)) return null; // 全部是 null，畫出來也是一片空白

  const labels = dates.map(d => String(d).slice(5).replace('-', '/')); // 'YYYY-MM-DD' → 'MM/DD'
  const config = {
    type: 'line',
    data: {
      labels,
      datasets: [{
        label: 'AI 能見度指數（7 日移動平均）',
        data: ma,
        borderColor: '#0F9E7A',
        backgroundColor: 'rgba(15,158,122,0.12)',
        fill: true,
        pointRadius: 0,
        borderWidth: 3,
        tension: 0.3,
        spanGaps: true // 缺日的 null 用連線帶過，不要斷成一截一截
      }]
    },
    options: {
      title: { display: true, text: `近 ${dates.length} 天 AI 能見度指數走勢`, fontSize: 16 },
      legend: { display: false },
      scales: { yAxes: [{ ticks: { min: 0, max: 100, fontSize: 12 } }], xAxes: [{ ticks: { fontSize: 11 } }] }
    }
  };

  const params = new URLSearchParams({
    c: JSON.stringify(config),
    w: '800', h: '400', bkg: 'white', f: 'png'
  });
