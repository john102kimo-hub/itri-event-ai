...
// timeoutMs（批次 60，選填）：只有「純加分、失敗也無所謂」的呼叫會傳。
//
// ⚠️ 刻意**不**給預設值，也就是說 reply／push 這些真正在送訊息的呼叫維持原本「沒有
// signal」的行為。批次 57 把整條路徑的 fetch 都補上逾時，唯獨漏了這個檔案——那是真的
// 缺口，但補在「送訊息」上要另外想清楚秒數（訊息沒送出去等於記者已讀不回，跟一個
// 動畫沒跑出來完全不是同一個量級的代價），不該夾在一個加動畫的修改裡順手改掉。
// 這裡只讓加分型的呼叫能自己設一個短逾時，見 startLoading()。
async function callLineApi(path, body, { timeoutMs } = {}) {
...
// 把活動照片欄位推成 LINE image 訊息（最多 4 張），走 push。
// 批次 83 起這支只剩 1 對 1 的退路：照片平常跟文字答案同一則 reply 送出（見下面
// replyTextWithImages()），那則被拒時 1 對 1 才另外 push 照片——群組不 push，因為
// 群組的 push 是照成員人數計費的。
...
// quoteToken（批次 83，選填）：群組裡回答時「引用」記者那一則問題——多人輪流發問、
// 答案又要等十幾秒才出來，不引用的話沒人看得出這段是在回誰的哪一題。
export function replyMessage(replyToken, text, quickReplyItems, quoteToken) {
...
// 每月免費 200 則，1 對 1 的記者量級用不完；重點是記者不會「問了沒下文」。
// ⚠️ 群組的 push 是照「群組人數」計則數的（推一次到 30 人的群組算 30 則），這條退路
// 在群組裡很貴——群組的路徑要盡量讓一次請求只用掉一次 reply（見 replyTextWithImages()）。
// quickReplyItems 是選填的按鈕文字陣列，兩條路徑（reply／push）都會帶上，
// 記者不會因為剛好遇到 reply 失效退回 push 就少看到按鈕。
//
// ⚠️ 帶了 quoteToken 而 reply 失敗時，先拿掉引用、用同一個 reply token 再送一次，才退到
// push（批次 83）。引用是加分：token 過期、那則訊息的類型不能引用，LINE 都會回 400——
// 不能因為「引用不了」就連答案一起送不出去，也不能因此改走 push（群組的 push 是照成員
// 人數計費，見 replyTextWithImages() 的說明）。失敗的請求不會用掉 reply token。
export async function replyOrPush(replyToken, userId, text, quickReplyItems, { quoteToken } = {}) {
  try {
    let res = await replyMessage(replyToken, text, quickReplyItems, quoteToken);
    if (!res.ok && quoteToken) {
      console.error('LINE reply（含引用）失敗，拿掉引用再試:', res.status, await res.text().catch(() => ''));
      res = await replyMessage(replyToken, text, quickReplyItems);
    }
    if (res.ok) return true;
...
// 文字答案＋活動照片，**同一則 reply 一起送**（批次 83）。
//
// 以前照片固定另外 push（見 pushImages() 的說明）。在 1 對 1 那只是多用一則免費額度；
// 在**群組**就貴了：LINE 官方帳號的訊息則數是「傳送對象人數 × 次數」，push 到一個
// 30 人的群組就算 30 則（一次請求裡放幾張照片不影響則數），而 reply 不計入額度。輕用量
// 方案一個月 200 則，30 人的群組問到第七次照片就超過——之後整個帳號的 push 全部失敗
// （包括 reply token 失效時的退路）。
//
// 做法：reply 一次送「文字＋最多 4 張照片」（LINE 一次最多 5 則）。LINE 送出當下只檢查
// 網址格式、不會去抓圖，所以一張圖打不開不會讓整則失敗；真的被拒（格式不合）就退回
// 只送文字（同一個 reply token，失敗的請求不會用掉它），1 對 1 再照舊另外 push 照片，
// 群組就不 push——文字答案裡本來就逐張列了下載網址（見 lib/prompt.js formatImages）。
//
// ⚠️ 快速回覆按鈕要掛在**最後一則**上：LINE 只顯示 messages 陣列最後一則的 quickReply，
// 掛在文字那則的話，後面接了照片，按鈕就不見了。
export async function replyTextWithImages(replyToken, userId, text, quickReplyItems, images, { quoteToken, isGroup = false } = {}) {
  const imgs = buildImageMessages(images, 4);
  if (!imgs.length) return replyOrPush(replyToken, userId, text, quickReplyItems, { quoteToken });
  const textMsg = { type: 'text', text: truncate(text) };
  if (quoteToken) textMsg.quoteToken = quoteToken;
  const quickReply = buildQuickReply(quickReplyItems);
  const last = { ...imgs[imgs.length - 1], ...(quickReply ? { quickReply } : {}) };
  const messages = [textMsg, ...imgs.slice(0, -1), last];
  try {
    const res = await callLineApi('/message/reply', { replyToken, messages });
    if (res.ok) return true;
    console.error('LINE reply（文字＋照片）失敗，改成只送文字:', res.status, await res.text().catch(() => ''));
  } catch (e) {
    console.error('LINE reply（文字＋照片）例外，改成只送文字:', e.message);
  }
  const ok = await replyOrPush(replyToken, userId, text, quickReplyItems, { quoteToken });
  if (!isGroup) {
    try {
      const res = await pushImages(userId, images);
...
//
// ⚠️ 3 秒逾時（批次 60）：這支從「只有活動問答會呼叫」變成「每一則 1 對 1 訊息都會
// 呼叫」（見 api/line.js handleEvent 的說明），它擋在記者那一題的最前面。原本完全
// 沒有 signal，LINE API 只要「連上了但不回應」，記者等到的就不是慢一點的答案，而是
// 整題卡死——批次 57 講的那種沉默，換一個入口重演一次。
// 動畫是加分、答案是本體，所以寧可放棄動畫也不能讓它拖住答案：3 秒不回就放棄往下走。
...
export async function getProfile(userId) {
  try {
    ...
    const res = await fetch(`${LINE_API}/profile/${encodeURIComponent(userId)}`, {
      ...
      // 職員登入流程會等這一支；LINE 不回應時寧可不顯示名字，也不能讓登入回覆一起卡住（批次 82）
      signal: AbortSignal.timeout(8_000)
    });
