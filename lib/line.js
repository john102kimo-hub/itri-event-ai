...
// 送一張單張圖片（目前只有 GEO 趨勢圖在用，見 lib/staff.js buildGeoTrendChartUrl()）。
// 跟 pushImages() 不共用同一支的原因：pushImages() 是「活動照片欄位」那套，特地做了
// jpg/png 副檔名篩選，因為那個欄位是同仁手動貼的、什麼網址都可能出現。這裡的網址是
// 我們自己組出來的 QuickChart.io 圖表連結，路徑上沒有 .png 副檔名（是 /chart?...&f=png
// 這種查詢字串），套用同一套副檔名正則只會把自己的圖擋掉——來源可信、不需要那層篩選。
