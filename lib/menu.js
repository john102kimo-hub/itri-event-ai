...
export const HELP_TEXT = [
  ...
  '',
  // 30 秒的動畫版使用說明（public/guide.html）。放在最前面是刻意的：整份 HELP_TEXT
  // 有四十幾行，記者在手機上多半只看得到前幾行就滑過去了；「看 30 秒影片」比「讀完
  // 四十行」的門檻低得多，願意讀的人往下滑照樣讀得到。
  // ⚠️ 網址一定要用自家站台，不要用任何內部或需要登入的連結——這一行會直接送到
  // 記者手機上，點不開的連結比沒有連結更糟。
  // 影片會直接播在這則文字的上面（見 api/line.js handleMetaIntent 的 'help' 分支）。
  // ⚠️ 措辭刻意寫成「影片沒出來也讀得通」：LINE 送出時回 200、之後才去抓媒體檔，抓
  // 失敗它不會通知我們，畫面上就是「只有文字、沒有影片」。上一版寫「⬆ 上面那支影片
  // 就是完整說明」，影片真的沒出來時，這句話變成指著一個不存在的東西。
  '▶ 30 秒看完：上面的影片，或開這一頁',
  ...
  '【在群組裡問我】',
  '　我只有被叫到才會說話，平常不插話。',
  '　@ 我一下再接著打問題，或直接用「米亞」開頭，',
  '　例如「米亞 最近有哪些活動」。',
  '　（有些人的 @ 選單裡找不到我，用「米亞」開頭就可以。）',
  '　群組看不到下面那排圖文選單（那是 1 對 1 專屬的），',
  '　所以每則答案底下的按鈕列最前面固定放了',
  '　🏠 回首頁 跟 📅 其他活動，隨時可以換一場或換個功能。',
  '',
