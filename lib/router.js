...
const VALID_INTENTS = ['calendar', 'qa', 'industry_trend', 'other'];
...
- "industry_trend"：問整體產業趨勢、市場現況、技術發展方向，不是在問某一場記者會的內容，例如「半導體最近有什麼趨勢」「AI 晶片產業現況如何」「機器人市場現在怎麼樣」——跟下面活動清單裡任何一場都無關，是在問更廣泛的產業／技術走向
- "other"：閒聊、打招呼、政治或立場評論、與活動及產業趨勢都完全無關的問題、或訊息意義不明

以下是目前可查詢的活動清單（id｜名稱｜日期｜類型｜資料狀態），由新到舊：
${list}

只回傳這個格式的 JSON：
{"intent":"calendar|qa|industry_trend|other","event_ids":["符合的活動 id，qa 才需要，從清單裡的 id 挑，最多 3 個，信心不足就多列幾個"],"confidence":"high|low"}`;
  ...
  if (currentEvent) {
    system.push({
      type: 'text',
      text: `目前這個對話正在問的是「${currentEvent.name}」這一場。訊息如果沒有明確指向清單中的其他場次，但看起來像是在延續、追問這場活動的內容（即使沒有再提到活動名稱），請判成 "qa"，event_ids 填這一場的 id；如果訊息是在問整體產業趨勢、市場現況（不是這場活動本身的內容），請判成 "industry_trend"，不要因為目前有綁定就硬塞成 "qa"；只有訊息明顯在聊完全無關的事、打招呼，才判為 "other"。`
    });
  }
