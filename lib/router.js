...
{"intent":"calendar|qa|other","event_ids":["符合的活動 id，qa 才需要，從清單裡的 id 挑，最多 3 個，信心不足就多列幾個"],"confidence":"high|low"}`;
  ...
  if (currentEvent) {
    system.push({
      type: 'text',
      text: `目前這個對話正在問的是「${currentEvent.name}」這一場。訊息如果沒有明確指向清單中的其他場次，但看起來像是在延續、追問這場活動的內容（即使沒有再提到活動名稱），請判成 "qa"，event_ids 填這一場的 id；只有訊息明顯在聊完全無關的事、打招呼、詢問跟這場活動及清單裡任何一場都無關的內容，才判為 "other"。`
    });
  }
