...
// 拆成「即將／近期」跟「已結束」兩組，各自照時間排序——formatCalendarReply()（文字列表）
// 跟 api/line.js 組快速回覆按鈕都要用同一份排序，抽出來才不會兩邊順序兜不起來。
export function splitUpcomingPast(cards) {
...
export function formatCalendarReply(cards) {
  ...
  const { upcoming, past } = splitUpcomingPast(cards);

  // 記者版的 cards 永遠不含 draft／archived，這兩個標籤不會出現；職員版
  // （buildAllCalendarCards）會，讓職員一眼看出哪幾場還沒發布或已下架。
  const fmt = c => {
    const tag = c.status === 'draft' ? '　🔒未發布' : c.status === 'archived' ? '　（已下架）' : '';
...
// 給快速回覆按鈕用：從行事曆卡片裡挑活動名稱，即將／近期優先、其次近期已結束，
// 跟 formatCalendarReply() 文字列出的順序一致。只挑「有資料」的場次——沒有 kb 的
// 場次點了按鈕也問不出內容，不占按鈕位置（LINE 上限 13 顆，要留給問得出東西的）。
export function calendarQuickReplyItems(cards, limit = 8) {
  const { upcoming, past } = splitUpcomingPast(cards.filter(c => c.has_kb));
  return [...upcoming, ...past].slice(0, limit).map(c => c.name);
}
