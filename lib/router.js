...
export async function routeIntent(userText, cards, { currentEventId, currentTopic } = {}) {
...
{"intent":"calendar|qa|industry_trend|tech_query|other","event_ids":["符合的活動 id，qa 才需要，從清單裡的 id 挑，最多 3 個，信心不足就多列幾個"],"tech_keyword":"tech_query 才需要，從問句抽出的簡短技術關鍵字","confidence":"high|low"}`;
  ...
  const TOPIC_HINTS = {
    industry_trend: '上一則我剛回答完的是「產業趨勢」的問題（資料來源是 IEK 產業情報網的免費焦點清單）。這則訊息如果只是一個產業或技術名詞（例如「太空」「半導體」「光通訊」）、或是「那ＸＸ呢」這種省略式的追問，那就是記者在接著同一個話題換個領域問，請判成 "industry_trend"，即使目前有綁定活動也一樣。只有訊息明確在問某一場活動的內容、明確提到「工研院」自己的技術，或明顯在聊完全無關的事，才照前面的規則判成 "qa"／"tech_query"／"other"。',
    tech_query: '上一則我剛回答完的是「工研院自己在某項技術上的研發成果」的問題（資料來源是工研院官網新聞中心）。這則訊息如果只是一個技術名詞、或是「那ＸＸ呢」這種省略式的追問，那就是記者在接著同一個話題換個技術問，請判成 "tech_query"（並把那個技術名詞填進 tech_keyword），即使訊息裡沒有再提到「工研院」、即使目前有綁定活動也一樣。只有訊息明確在問某一場活動的內容、明確在問整體產業／市場趨勢，或明顯在聊完全無關的事，才照前面的規則判成 "qa"／"industry_trend"／"other"。'
  };
  if (TOPIC_HINTS[currentTopic]) system.push({ type: 'text', text: TOPIC_HINTS[currentTopic] });
