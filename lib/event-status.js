...
// ── 「活動與進度」總覽 ─────────────────────────────────────────────────
// 列哪些場：跟職員的「哪一場」候選同一個原則——草稿一律列（還沒發布＝還有事要做），
// 封存不列，進行中／已結束的只列「還沒到、或日期未定」的。排序：有日期的照日期、
// 日期未定的排最後。
export function progressCandidates(rows, todayIso) {
  const list = (rows || [])
    .filter(r => r && r[0] && r[1] && r[4] !== 'archived')
    .map(r => eventChecklist(r, todayIso))
    .filter(c => c.status === 'draft' || c.daysLeft === null || c.daysLeft >= 0);
