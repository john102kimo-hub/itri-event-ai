...
async function fetchNewsForKeywordOnce(keyword, hours) {
  // 10 秒逾時（批次 82）：Google 那端卡住時，這個議題算「這次沒查到」（failedKeywords），
  // 不要拖著整支 /api/geo 等到 60 秒被砍、所有議題一起白查
  const res = await fetch(buildUrl(keyword, hours), { headers: { 'User-Agent': 'Mozilla/5.0' }, signal: AbortSignal.timeout(10_000) });
