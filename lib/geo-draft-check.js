...
import { BRAND_ALIAS_RE } from './geo-orgs.js';

// 「有沒有寫到工研院」認所有寫法（批次 82）：以前只認「工研院」三個字，稿子開頭寫
// 「工業技術研究院」或英文稿寫「ITRI」，這兩項就判不及格——呈核頁上長官看到的是
// 「開頭沒寫工研院」，明明寫了。
const hasBrand = (s) => BRAND_ALIAS_RE.test(String(s || ''));
const MAX_PARA = 400;
...
export function bestQuotable(text) {
  ...
  const ok = sentences.filter((s) => hasBrand(s) && NUMBER_RE.test(s) && s.length >= 15 && s.length <= 80);
...
export function checkGeoDraft(text, { keyword = '' } = {}) {
  ...
  const checks = [
    { key: 'lead_brand', label: '第一段就寫出「工研院」（當主詞；寫全名「工業技術研究院」或 ITRI 也算）',
