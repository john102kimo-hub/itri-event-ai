...
// 實際回報（附截圖）：按鈕引導流程（api/line.js handleTechQueryMessage()）問記者
// 「請問您想了解工研院哪一項技術呢？」之後，記者不見得會照範例打單一技術名稱，
// 常常打一整句（例如「最近的國際合作」）。工研院官網的搜尋是接近精準比對，整句
// 話（含「最近」「的」這類語助詞）常常查不到，去掉語助詞剩下的核心關鍵字才查得
// 到——實測（curl 對真的官網）：「最近的國際合作」查 0 筆，去掉「最近」「的」之後
// 的「國際合作」查得到 10 筆。
//
// 自然語言路徑（routeIntent() 判成 tech_query，見 lib/router.js）已經請 AI 順手抽
// 一次關鍵字，但 AI 抽詞失敗（沒 API key、逾時、判斷失準）時一樣會退回整句原話
// ——兩條路徑最後都會走到 fetchItriNews()，索性在這裡統一加一層保底，不逼每個
// 呼叫端都要自己保證給的一定是乾淨關鍵字。
//
// 只挑最常見、拿掉了幾乎不會誤傷技術名稱本身的語助詞／泛稱／標點，不做真的分詞
// ——這不是要精準模擬語言理解，只要「查無結果時退而求其次再試一次」這個保底
// 動作，抽得不夠乾淨（殘留一兩個字）沒關係，反正 fetchItriNewsOnce() 查到的東西
// 一樣會透過 formatNewsForPrompt() 讓 AI 自己判斷相不相關，抽詞不是最後一道防線。
const TECH_QUERY_FILLER_RE = /工研院|最近|最新|有什麼|有沒有|新聞|進展|技術上|請問|想了解|想問|想知道|的|嗎|呢|[?？！!。，,]/g;
export function stripTechQueryFiller(text) {
  return String(text || '').replace(TECH_QUERY_FILLER_RE, '').replace(/\s+/g, '').trim();
}

// 打一次工研院官網新聞中心的清單頁——查詢失敗（非 2xx、fetch 本身丟例外）一律
// 讓例外往外拋，交給 fetchItriNews() 統一接住，這支自己不接。
// ⚠️ 逾時（批次 57）：這是**外部網站**，我們對它的可用性零控制權，而它卡住的代價
// 是整支 LINE function 被 Vercel 在 60 秒砍掉、記者連本來答得出來的那則都收不到
// （見 api/line.js ANSWER_TIMEOUT_MS 的說明）。10 秒對一個伺服器端渲染的清單頁很寬鬆，
// 超過就當它今天不在，回「暫時抓不到」比拖垮整則回覆好。
const ITRI_FETCH_TIMEOUT_MS = 10_000;
async function fetchItriNewsOnce(kw) {
  const url = kw ? `${ITRI_NEWS_BASE}&Page=1&keyword=${encodeURIComponent(kw)}` : `${ITRI_NEWS_BASE}&Page=1`;
  const res = await fetch(url, {
    headers: { 'User-Agent': 'Mozilla/5.0' },
    signal: AbortSignal.timeout(ITRI_FETCH_TIMEOUT_MS)
  });
...
export async function fetchItriNews(keyword) {
  ...
  try {
    const items = await fetchItriNewsOnce(kw);
    if (items.length || !kw) return { ok: true, items };

    // 第一次查無資料——去掉語助詞再試一次；stripped 是空字串（整句都是語助詞）
    // 或跟原字串一樣（沒東西可去掉）就不用白跑一趟，直接回查無資料。
    const stripped = stripTechQueryFiller(kw);
    if (!stripped || stripped === kw) return { ok: true, items };
    // 重試本身失敗（剛好網路閃斷）不算「抓取失敗」——第一次已經證實官網連得上，
    // 這裡老實回報「查無資料」（第一次查到的空清單）比改口說「暫時抓不到資料」
    // 更貼近實情，見上面 ok/items 的說明。
    const retryItems = await fetchItriNewsOnce(stripped).catch(() => null);
    return { ok: true, items: retryItems || items };
  } catch (e) {
