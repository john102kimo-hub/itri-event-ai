...
export async function getMemories() {
  ...
  try {
    ...
    // 批次 84：分頁根本還沒建（第一次有人用「記住」才會建）——正式站實測每一題 AI 回答都
    // 白讀一次、失敗一次，Vercel Logs 被這行洗滿，蓋掉真正的錯誤（那天 AI 金鑰失效就是
    // 夾在這堆裡面）。直接把空分頁建起來，之後讀得到空表、照常快取，不再每題失敗。
...
// ⚠️ 批次 76 盤點實測：同仁想改活動資料時打「改成下午兩點開始」「請改一下智慧醫療的地點」，
// 會命中上面那條，被當成**全站語氣規則**——確認句又沒講會套用到所有場次，按一次
// 「✅ 記起來」，之後每一場的每一題模型都會收到「改成下午兩點開始」。
// 句子一碰到活動資料的欄位（時間、地點、日期、聯絡人⋯⋯）就不是「說話方式」，不收進 style。
// ⚠️ 時間只認「下午兩點」「10 點」「14:00」「點開始」這類：「短一點」「多一點」的「一點」是
// 「稍微」，不是時間，用裸的「數字＋點」會把「以後回答短一點」誤擋掉。
const FIELD_CHANGE_RE = /地點|場地|會場|時間|日期|幾點|(上午|下午|早上|晚上|中午)\s*[0-9０-９一二三四五六七八九十兩]+\s*點|[0-9０-９]+\s*點|[0-9０-９]{1,2}[:：][0-9０-９]{2}|點開始|延期|延後|提前|改期|改到|改在|聯絡人|窗口|電話|手機|名稱|活動名/;
export function looksLikeFieldChange(text) {
  return FIELD_CHANGE_RE.test(String(text || ''));
}
...
export function parseMemoryCommand(text) {
  ...
  if (NATURAL_STYLE.test(s) && looksLikeFieldChange(s)) return { kind: 'field_hint', text: s };
