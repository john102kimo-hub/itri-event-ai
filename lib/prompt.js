...
// 回答不出東西。
//
// 回報的意見：chips（快速提問按鈕，LINE quick reply 跟網頁版都在用）沒有跟著換，
// 活動前記者點「這次活動的主要發表內容是什麼？」這種預設問句，AI 只能照實說「這部分
// 我沒有資料」——不是壞掉，但沒有用，等於問答換了內容、按鈕卻還留著問不到的舊問題。
// 同仁可以另外填一份「活動前快速提問」（invite_letter_chips），活動前優先用這份；
// 沒填就退回原本的 chips（不會讓活動前的按鈕整排消失）。
export function resolveEventContent(event) {
  if (isPreEventMode(event)) {
    return {
      ...event,
      knowledge_base: event.invite_letter,
      images: '',
      chips: event.invite_letter_chips || event.chips
    };
  }
  return event;
}
...
// ── 活動基本資料（批次 72）───────────────────────────────────────────────
// 盤點時實測：記者綁定一場之後問「這場幾點開始」「在哪裡辦」「開始了嗎」「新聞聯絡人
// 是誰」——這幾題是現場最常被問的，而後台明明有「日期／時間／地點／新聞聯絡人」四個
// 欄位（events F、L、M、O），**卻從來沒有交給答題的模型**：rowToEvent() 沒讀 L、M 欄，
// buildSystemPrompt() 只放知識庫全文。新聞稿內文沒寫到時間地點（活動後發的新聞稿通常
// 不寫），模型只能照規則回「這部分我沒有資料」，接著 NO_DATA 標記還會拿「活動時間」
// 去官網新聞中心補查一輪——資料就在我們自己的表裡，卻繞了一大圈說不知道。
//
// 另外附上「現在時間」：模型不知道今天幾號，「開始了嗎」「還有多久」答不出來，也可能
// 把已經辦完的活動講成未來式。
//
// ⚠️ 這段刻意不併進 buildSystemPrompt()：那份的輸出要跟網頁版逐 byte 相同（見檔頭），
// 而且它吃 prompt cache——現在時間每分鐘都在變，要放在呼叫端的第二個 system 區塊。
// ⚠️ 活動前（邀請函模式）照樣給：日期、時間、地點、聯絡人本來就是邀請函會寫、也是
// 「尚未舉行的場次只能提供」的那幾項（見 LINE-PLAN.md 第 6 節）。
const WEEKDAYS = ['日', '一', '二', '三', '四', '五', '六'];
function twDateParts(now) {
  const ymd = now.toLocaleDateString('sv-SE', { timeZone: 'Asia/Taipei' }); // YYYY-MM-DD
  const hm = now.toLocaleTimeString('en-GB', { timeZone: 'Asia/Taipei', hour: '2-digit', minute: '2-digit', hour12: false });
  return { ymd, hm };
}
...
// F 欄只有「手動填的日期」才算活動日期。沒填日期時系統寫進去的是建立時間戳記
// （「2026/6/18 下午11:06:04」），那不是活動日——後面還帶著時間的一律不採用。
function strictEventDate(raw) {
  const m = String(raw || '').trim().match(/^(\d{4})[-/](\d{1,2})[-/](\d{1,2})$/);
  if (!m) return '';
  return `${m[1]}-${m[2].padStart(2, '0')}-${m[3].padStart(2, '0')}`;
}

export function formatEventBasics(event, now = new Date()) {
  const date = strictEventDate(event?.event_date);
  const time = String(event?.event_time || '').trim();
  const venue = String(event?.venue || '').trim();
  const contact = String(event?.press_contact || '').trim();
  if (!date && !time && !venue && !contact) return '';

  const { ymd, hm } = twDateParts(now);
  const lines = [];
  if (date) {
    const days = Math.round((Date.parse(date + 'T00:00:00Z') - Date.parse(ymd + 'T00:00:00Z')) / 86400000);
    const rel = days === 0 ? '——就是今天' : days > 0 ? `——還有 ${days} 天` : `——已經在 ${-days} 天前舉行`;
    lines.push(`・活動日期：${date}（星期${weekdayOf(date)}）${rel}`);
  }
  if (time) lines.push(`・時間：${time}`);
  if (venue) lines.push(`・地點：${venue}`);
  if (contact) lines.push(`・新聞聯絡人：${contact}`);
  lines.push(`・現在時間：${ymd}（星期${weekdayOf(ymd)}）${hm}（台灣時間）`);

  return `【活動基本資料】（主辦單位在後台填寫的正式欄位）
${lines.join('\n')}
記者問日期、時間、地點、新聞聯絡人，或活動開始了沒、還有多久時，直接用上面這幾行回答，不要說沒有資料；背景資料裡的說法跟這裡不一致時，兩個都列出來，並建議洽新聞聯絡人確認。已經舉行過的活動不要用未來式描述。`;
}
...
export function buildSystemPrompt(event, extraRules = []) {
  ...
  const rules = [
    ...
    ...extraRules,
    // 回報的意見：擺在規則清單中段時，模型（尤其 haiku）常常還是被周圍滿滿的中文背景
    // 資料／規則帶著走，記者打英文照樣收到中文回覆。這條刻意放在 rules 最後一條——
    // 全篇僅次於結尾警語的第二強位置——而且中英文都寫一遍、直接下判斷語言的具體
    // 動作，不要只當成眾多規則之一被平均掉。
    'IMPORTANT — Always reply in the SAME language as the reporter\'s latest question, regardless of what language the background data or these instructions are written in. If the question is in English, your entire answer (including the closing disclaimer) must be in English. If it\'s in Chinese, reply in Chinese. 請務必以記者「這一則提問」所使用的語言回答，不論背景資料或這份指示本身是什麼語言；記者用英文提問，整段回答（含結尾警語）都要用英文，不要中英夾雜、也不要自行切回中文。'
  ];
...
回答規則：
${rules.map(r => `- ${r}`).join('\n')}
- 每則回答的最後，務必另起一行加註警語，內容等同於「內容僅供參考，以${organizer}官網新聞稿或發言為準。」；若這則回答是用英文或其他語言回覆，警語也要翻成同一種語言（例如英文回覆時警語可寫成「For reference only; please confirm with ${organizer}'s official press release or spokesperson.」），不要在其他部分都用外語、卻只有這句警語留中文。`;
}
