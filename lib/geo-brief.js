...
function eventRow(ev) {
  ...
  const liftText = !ev.settled
    ? '30 天觀察期還沒到，暫不評斷留存'
    // 第 15–30 天是餘波期，不是基線（批次 72，見 api/geo.js eventFindings 的說明）——不寫「基線」兩個字
    : ev.lift === null ? '' : ev.lift > 0 ? `第 15–30 天比發稿前 +${ev.lift}` : `第 15–30 天回到原點（${ev.lift}）`;
...
export function buildGeoBriefFlex(statusData, seriesData, siteUrl) {
  ...
  const bubble = {
    ...
    footer: {
      ...
      contents: [
        // 批次 75：長官打「GEO 簡報給我」要的是投影片，不是儀表板首頁——直接開到簡報分頁
        { type: 'button', height: 'sm', style: 'primary', color: BRAND, action: { type: 'uri', label: '開啟能見度簡報', uri: `${siteUrl}/geo#deck` } },
