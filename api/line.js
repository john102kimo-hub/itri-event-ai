...
function lineExtraRules(event) {
  ...
  return [
    ...
    '如果上面的背景資料裡完全沒有可以回答這一題的內容（也就是你這則回覆的重點是「這部分我沒有資料」），請在回覆中另起獨立的一行，只放這個格式的標記：[[NO_DATA:關鍵詞]]，關鍵詞是記者這題真正想問的主題，2-6 個字的名詞（例如問「今年院士有誰」就填「院士」，問「得獎名單」就填「得獎名單」），不要填整句問句、不要填「沒有資料」這種描述。這行是給程式判讀用的、不會顯示給記者，放在哪一行都可以，不算進上面的行數限制；如果背景資料答得出這題，就完全不要加這一行。',
