...
export function toTraditionalTW(input) {
  ...
  for (const { re, to, keepPrev, keepNext } of S2T_CONTEXT) {
    out = out.replace(re, (ch, i, whole) => {
      ...
      return (keepPrev.includes(prev) || keepNext.includes(next)) ? ch : to;
    });
  }
  let hits = 0;
  ...
  if (hits) console.log(`[line] 出口轉繁體：換掉 ${hits} 個簡體字`);
  return out;
}
...
async function askAnthropic(systemPrompt, userText, history = [], { extraSystem = '' } = {}) {
  ...
  try {
    ...
    const text = (data.content || [])
      ...
      .map(b => b.text).join('\n').trim();
    // ⚠️ 兩道出口清理的順序：先清 Markdown 再轉繁體。反過來也可以，但固定一個順序
    // 才不會有人日後在中間插一段、結果只有一半的文字被處理到。
    return toTraditionalTW(stripMarkdownForLine(text)) || '抱歉，無法取得回應。';
  } catch (e) {
