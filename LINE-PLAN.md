...
- `lib/router.js`：組行事曆清單 → Haiku 一次呼叫回 `{intent, event_ids, confidence}`
- 四類意圖分別接上（第 4 節）
- `media_requests` 分頁 + `LINE_ADMIN_USER_ID` 推播通知
- 行事曆清單快取 60 秒（坑 4）
- **前置條件（沒做完不准開工）**：批次 1 的 `status` 三態與 `event_date` 已上線並驗收過
- **驗收**：
  1. 用**全新 LINE 帳號**（無任何綁定）問「這個月有哪些活動」→ 列得出來，且**不含任何 draft 場次**
  2. 拿 draft 場次的**活動全名**去問 → 必須答「沒有相關場次」，回答中不得出現該場任何內容。要多試幾種問法
  3. 問三個月前那場的細節 → 答得出來，結尾有時間戳警語
  4. 問未來場次的細節 → 只回基本資料 + 聯絡人，**不得預先釋出 kb 內容**
  5. 說「我想採訪 XX」→ 逐項追問 → 寫進 `media_requests`，使用者的 LINE 收到通知
  6. 問跨場次歸納題（「今年發表了幾項技術？」）→ 回「以下幾場提到」+ 分場原文，**不得自行總結**
  7. 全流程含路由在 20 秒內回覆完畢
